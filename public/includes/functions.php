...
function validateWhatsApp($whatsapp) {
    ...
    logSystem("[DEBUG] validateWhatsApp: original='{$original}', normalizado='{$whatsapp}'", 'DEBUG');
    // Aceita apenas 11 dígitos, terceiro dígito obrigatoriamente 9 (celular)
    if (preg_match('/^\d{2}9\d{8}$/', $whatsapp)) {
        logSystem("[DEBUG] validateWhatsApp: '{$whatsapp}' válido", 'DEBUG');
        return true;
    }
    logSystem("[DEBUG] validateWhatsApp: '{$whatsapp}' inválido", 'DEBUG');
    return 'O WhatsApp deve ser um número de celular válido no formato (99) 9XXXX-XXXX';
}
...
function processParticipantForm($postData, $sorteio, $camposConfig) {
    ...
    // Log temporário para depuração do valor recebido
    if (isset($postData['whatsapp'])) {
        logSystem("[DEBUG] processParticipantForm: whatsapp recebido='" . $postData['whatsapp'] . "'", 'DEBUG');
        $postData['whatsapp'] = preg_replace('/[^0-9]/', '', $postData['whatsapp']);
        logSystem("[DEBUG] processParticipantForm: whatsapp normalizado='" . $postData['whatsapp'] . "'", 'DEBUG');
    }
    ...
    $camposExtras = [];
    $data['campos_extras'] = []; // Inicializa para evitar erros no formulário
    if (!empty($camposConfig['campos_extras']) && is_array($camposConfig['campos_extras']) && isset($postData['campos_extras']) && is_array($postData['campos_extras'])) {
        foreach ($camposConfig['campos_extras'] as $campo) {
            if (empty($campo['nome'])) continue;

            $fieldName = $campo['nome'];
            $value = $postData['campos_extras'][$fieldName] ?? '';
            
            // Guarda o valor submetido para repopular o formulário em caso de erro
            $data['campos_extras'][$fieldName] = $value;

            if (!empty($campo['required']) && trim($value) === '') {
                $result['success'] = false;
                if (!isset($result['errors']['campos_extras'])) $result['errors']['campos_extras'] = [];
                $result['errors']['campos_extras'][$fieldName] = "O campo " . htmlspecialchars($campo['label']) . " é obrigatório.";
            }
            
            if (!empty($value)) {
                $camposExtras[$fieldName] = sanitizeInput($value);
            }
        }
    }
    
    if (!$result['success']) {
        $result['data'] = $data; // Garante que os dados submetidos voltem para o formulário
        return $result;
    }
